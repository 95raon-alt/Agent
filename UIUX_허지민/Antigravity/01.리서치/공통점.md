...
## 🔍 2. 200개 레퍼런스 종합 5대 공통 패턴 및 25대 대표 브랜드 매핑

```mermaid
graph TD
    A["200개 D2C 건기식 웹사이트 5대 공통 UI/UX"] --> B["1. 10초 컷 퀴즈 & 맞춤 큐레이션 (적용률 68%)"]
    A --> C["2. 3D 이중제형/캡슐 오브제 비주얼 (적용률 55%)"]
    A --> D["3. 가성비 저울 & 단품 vs 구독 비교 (적용률 48%)"]
    A --> E["4. 3줄 요약 카드 & 정보 아코디언 (적용률 72%)"]
    A --> F["5. 모바일 Thumb-Zone 핀 고정 탭 (적용률 82%)"]
```

---

### 1. 🎯 10초 컷 퀴즈 & 맞춤 큐레이션 UX (적용률: 68% / 136개사)
- **핵심 특징**: 상품 목록(Grid) 나열 대신 메인 상단에 "성별·연령·직장인 고민(피로, 수면, 숙취, 면역)" 3~5문항 퀴즈 배치하여 구매 결정 단순화.
- **A:FIT 적용점**: 메인 진입 10초 만에 직장인 상황별 맞춤 파우치 큐레이션 폼 연동.
...
### 2. 💎 3D 이중제형/캡슐 & 오브제 비주얼 (적용률: 55% / 110개사)
- **핵심 특징**: 의약품 약통 느낌을 탈피한 라이프스타일 데스크 오브제 연출 및 투명 이중제형(액상+정제) 3D 그래픽/패럴랙스 스크롤 증명.
- **A:FIT 적용점**: 이중제형 3D 투명 렌더링컷을 메인 히어로에 배치하여 기술력과 감성 브랜딩 동시 확보.
...
### 3. ⚖️ 가성비 저울 & 단품 vs 구독 비교 (적용률: 48% / 96개사)
- **핵심 특징**: 단품 5~10개 개별 구매 비용 vs 올인원 1포 구독 비용을 저울 그래픽과 1회분당 가격 계산표로 비교하여 가격 저항감 해소.
- **A:FIT 적용점**: 개별 구매 대비 A:FIT 1포 구독 시 절감되는 월 비용 저울 그래픽 제시.
...
### 4. 📋 3줄 요약 카드 & 정보 아코디언 위계 (적용률: 72% / 144개사)
- **핵심 특징**: 복잡한 성분표를 [주요 효능 - 원료 출처 - 임상 수치] 3줄 카드로 압축하고 세부 데이터는 3단 아코디언으로 정돈.
- **A:FIT 적용점**: 제품 상세 설명 시 3줄 요약 카드로 정보 피로도 원천 차단.
...
### 5. 📱 모바일 Thumb-Zone 핀 고정 구독 탭 (적용률: 82% / 164개사)
- **핵심 특징**: 엄지손가락 터치 범위(Thumb-Zone)에 퀴즈 진입, 파우치 빌더, 구독 주기 선택 버튼을 Sticky 고정하여 전환율 극대화.
- **A:FIT 적용점**: 하단 핀 고정 모바일 탭으로 구독 주기 선택 및 결제 동선 최소화.
...
## 💡 3. A:FIT 웹 UI/UX 최적 조합 가이드라인

1. **메인 히어로 & 브랜드 감성**: **Seed** (3D 이중제형 패럴랙스 모션) + **Ritual** (투명 캡슐 3D 비주얼)
2. **퀴즈 & 맞춤 파우치 구성**: **Rootine** (실시간 파우치 빌더) + **HUM Nutrition** (직장인 고민 칩 선택)
3. **가성비 시각화 & 가격 설득**: **AG1** (가성비 저울 비교 비주얼) + **Gainful** (모바일 핀 고정 Thumb-Zone 탭)
4. **성분 설명 & 정보 설계**: **Thorne** (3단 아코디언 위계) + **Pure Encapsulations** (3열 요약 아이콘 카드)
